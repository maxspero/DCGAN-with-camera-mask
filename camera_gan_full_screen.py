...
def process_webcam_image(img, x_scale, y_scale, mask_value, flip=True, threshold=0.5, normalize=False):
  ...
  flipped[flipped<=threshold] = mask_value
  if normalize:
    flipped = normalize_arr(flipped)
  return flipped
...
def normalize_arr(input):
  in_avg = np.sum(input)/input.size
  return input/in_avg
...
while is_capturing:
    try:    # Lookout for a keyboardInterrupt to stop the script
        ...
        if frame is not None:
            ...
            bucket_choice = max(bucket(process_webcam_image(frame, 1/20, 1/15, 0, threshold=.3), 1, 5, True) - 1, 0)
